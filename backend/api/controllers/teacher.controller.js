...
export default {
  ...
  async findStudent(req, res) {
    ...
    try {
      for (const data of studentData2) {
        ...
        studentData.dataValues.headTeacherName = teacherData.dataValues.name;
  ...
  async updateLogbook(req, res) {
    ...
    if (success[0] === 1) res.json({ message: "updates logbook successfully" });
  ...
  async getLessons(req, res) {
    ...
    lessons.forEach(function (el) {
      final_lessons.push(el.dataValues);
    });
    res.send(final_lessons);
  },
