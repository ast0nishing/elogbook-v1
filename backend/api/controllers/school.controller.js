  ...
  async createTimeTable(req, res) {
    ...
    try {
      for (const timeTable of timeTables) {
        ...
        const classExist = await database.class.findOne({
        ...
        const timetableExist = await database.timetable.findOne({
          where: {
        ...
        const newTimeTable = await database.timetable.create(timeTable);
  ...
  async createTeacher(req, res) {
    ...
    try {
      for (const teacher of teachers) {
        ...
        teacher.password = await argon2.hash(teacher.password, { salt });
        ...
        await database.teacher.create(teacher);
      }
      if (
        alreadyExist.length === 0 &&
        missingInfo.length === 0 &&
  ...
  async createClass(req, res) {
    ...
    try {
      for (const aClass of students) {
        ...
        await database.class.create(aClass);
      }
      if (
        alreadyExist.length === 0 &&
        missingInfo.length === 0 &&
  ...
  async createStudent(req, res) {
    ...
    try {
      for (const student of students) {
        ...
        const studentExist = await database.student.findOne({
        ...
        const salt = randomBytes(32);
        student.password = await argon2.hash(student.password, { salt });
        ...
        const newStudent = await database.student.create(student);
        if (student.classId) {
          const classExist = await database.class.findOne({
  ...
  async classAddTeacher(req, res) {
    ...
    try {
      for (const aClass of classes) {
        ...
        const targetClass = await database.class.findOne({
          where: {
