...
export default {
  ...
  async editStudent(req, res) {
    ...
    if (studentData.dataValues.schoolId == req.user.id) {
      ...
      if (data[0] === 1) res.json({ message: "edit student successfully" });
  ...
  async getSelf(req, res) {
    ...
    console.log(schoolData.dataValues.password);
  ...
  async editTeacher(req, res) {
    ...
    if (
      ...
      if (data[0] === 1) res.json({ message: "edit teacher successfully" });
    ...
    for (const data of classData2) {
      if (data.dataValues.schoolId == req.user.id) {
        for (const studentData of data.dataValues.students) {
          ...
          studentData.dataValues.studentId = studentData.dataValues.idSchool;
          studentData.dataValues.password = "vtn";
