...
export default {
    ...
    async createClassAddStudent(req, res) {
        ...
        try {
            ...
            if (aClass.students) {
                ...
                if (!invalidTeacherId && !nonExistTeacher) {
                    if (
                        ...
                        return res.status(httpStatus.OK).json({
                            class: 'Create Class and add Head Teacher and add all students success',
                        });
                    }
                    return res.status(httpStatus.BAD_REQUEST).json({
            ...
            return res.status(httpStatus.BAD_REQUEST).json({
                class: 'Create Class sucess but fail add Head Teacher because of invalid or non exist teacher',
                'Invalid Teacher ID': invalidTeacherId,
                'Nonexist Teacher': nonExistTeacher,
            });
        } catch (err) {
